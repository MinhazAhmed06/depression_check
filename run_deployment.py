...
@click.option(
    ...
    help = '''Optionally you can choose to only run the deployment pipeline to train and deploy a model ('deploy'),
            or to only run prediction against the deployed model ('predict'). By default both will run
            ('deploy_and_predict')''',
)
@click.option(
    '--min-accuracy',
...
def run_deployment(config: str, min_accuracy: float):
    ...
    if existing_services:
        ...
        print('NO mlflow prediction server is currently running. The deployment pipeline must ' \
            'run first to train a model and deploy it. Execute the same command with "--deploy" argument ' \
            'to deploy a model')
